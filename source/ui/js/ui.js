...
/**
 * To whomever takes a look at this script,
 *
 *
 *
 *
...
class BackEndSocket {
    ...
    send(prefix, payload) {
        if (this.#websocket.readyState === WebSocket.OPEN)
            ...
            console.warn(
                "ERROR: Tried sending message to backend, but websocket is not open."
            );
        if (prefix !== "AUTH") {
...
async function main() {
    window["console-inputbox"].textContent = ""; // shorthand for document.getElementById
    initializeTemplates();
    intializeNavBar();
    ...
    initializeClearConsoleButton();
    initializeMonologueButton();
...
function sendInput() {
    ...
    if (content === "") return;
    if (input_box !== null) input_box.textContent = "";
    BackEnd.send(content.startsWith("!") === true ? "COMMAND" : "MESSAGE", {
        text: content,
    });
}
...
function initializeClearConsoleButton() {
    let btn = document.getElementById("control-clearconsole");
    if (btn === null) return;

    let console = document.getElementById("console-terminal");
...
function initializeMonologueButton() {
    let btn = document.getElementById("control-monologue");
    if (btn === null) return;
...
function initializeNewPresetButton() {
    ...
    add_btn.onclick = () => {
        ...
        if (
            name_input?.firstElementChild === null ||
            name_input?.firstElementChild === undefined
        )
            return;
        if (
            "value" in name_input?.firstElementChild &&
            typeof name_input?.firstElementChild.value === "string"
        ) {
...
function initializePauseButton() {
    ...
    pause_button.onclick = () => {
        ...
        if (pause_button === null) return;
        // @ts-ignore
        State.config["_"]["paused"]["value"] =
            !pause_button?.classList.contains("important");
        if (State.config["_"]["paused"]["value"] === true) {
            if (pause_button.classList.contains("important") === false)
                pause_button.classList.add("important");
        } else {
...
function initializeInputSendButton() {
    ...
    if (send_button !== null) send_button.onclick = sendInput;
    let input_box = document.getElementById("console-inputbox");
    if (input_box !== null)
...
function initializeTemplates() {
    ...
    for (let template of template_tags) {
        ...
        /** @type {HTMLElement[]} */ // @ts-ignore
        const templated_elements = Array.from(
            document.getElementsByTagName(template_name)
        );
...
function initializeParamNavBar() {
    /** @type {HTMLElement[]} */ // @ts-ignore
    const param_pages = Array.from(
        document.getElementsByClassName("param-section")
    );
        ...
        case "CONSOLE_DEBUG":
            {
                if (State.config === undefined) return;
                if (
                    State.config["behaviour"]["additional_logs"]["value"] ===
                    true
                )
                    Terminal.addConsoleMessage(
                        payload["text"],
                        payload["color"]
                    );
            }
...
function saveCharacter() {
    /** @type { HTMLSelectElement } */ // @ts-ignore
    let char_select = document.getElementById(
...
function generateAuth() {
    ...
    if (accounts_page !== null)
        for (let entry of Array.from(accounts_page?.children)) {
            ...
            let entry_name = split_entry_name?.slice(1, undefined).join(" ");
...
function setupParamsPage(config) {
    ...
    while (parameters_page_wrapper?.firstElementChild !== null)
        parameters_page_wrapper?.removeChild(
            parameters_page_wrapper?.firstElementChild
        );
        ...
        for (let [param, value] of Object.entries(config[field])) {
            ...
            switch (value["type"]) {
                ...
                case "select":
                    {
                        ...
                        if (field === "devices") val.setAttribute("test", true);
                        new_param.appendChild(val);
                    }
                    break;
                case "list":
                ...
                case "vts_emotions_list":
                    {
                        let val = document.createElement(
...
function generateConfig(config) {
    ...
    for (let page of Array.from(parameters?.children)) {
        ...
        if (config[page_name] === undefined) {
            console.error(
                `CRITICIAL ERROR: Could not find field "${page_name}" in config object.`
            );
            return;
        }
        ...
        for (let entry of Array.from(page.children)) {
            ...
            if (config[page_name][param_name] === undefined) {
                console.error(
                    `CRITICIAL ERROR: Could not find field "${page_name}.${param_name}" in config object.`
                );
                return;
            }
                ...
                case "tmpl-vts-emotes-list-input":
                    {
                        ...
                        for (let entry of Array.from(list.children)) {
                            ...
                            let talk_list =
                                talk_seq_line.firstElementChild
                                    ?.firstElementChild;
                            if (talk_list === undefined || talk_list === null)
                                continue;
                            for (let entry of Array.from(talk_list.children)) {
                                ...
                                // @ts-ignore
                                obj.talking_hotkey_sequence.push(
                                    item.textContent
                                );
                            }

                            let idle_list =
                                idle_seq_line.firstElementChild
                                    ?.firstElementChild;
                            if (idle_list === undefined || idle_list === null)
                                continue;
                            ...
                            let reset_list =
                                reset_seq_line.firstElementChild
                                    ?.firstElementChild;
                            if (reset_list === undefined || reset_list === null)
                                continue;
                            for (let entry of Array.from(reset_list.children)) {
                                ...
                                // @ts-ignore
                                obj.reset_hotkey_sequence.push(
                                    item.textContent
                                );
                            }
                            value.push(obj);
                    ...
                    break;
                case "tmpl-vts-keywords-list-input":
                    {
                        /** @type {HTMLElement} */ // @ts-ignore
                        let list_wrapper = param_element.firstElementChild;
                        /** @type {HTMLElement} */ // @ts-ignore
                        ...
                        for (let entry of Array.from(list.children)) {
                            ...
                            let sequence_line = entry.children[2]?.children[1];

                            if (keywords_line === undefined) continue;
                            if (sequence_line === undefined) continue;

                            let keyword_list =
                                keywords_line.firstElementChild
                                    ?.firstElementChild;
                            if (
                            ...
                            let sequence_list =
                                sequence_line.firstElementChild
                                    ?.firstElementChild;
                            if (
                                sequence_list === undefined ||
                                sequence_list === null
                            )
                                continue;
                            for (let entry of Array.from(
                                sequence_list.children
                            )) {
                                let item = entry.firstElementChild;
                                if (item === null) {
                                    console.warn("ERROR: item was null.");
                                ...
                                // @ts-ignore
                                obj.hotkey_sequence.push(item.textContent);
                            }
                            value.push(obj);
                        }
                        config[page_name][param_name]["value"] = value;
                        ...
                        for (let entry of Array.from(list.children)) {
                            ...
                            let keywords_list =
                                keywords_line.firstElementChild
                                    ?.firstElementChild;
                            if (
                                keywords_list === undefined ||
                                keywords_list === null
                            )
                                continue;
                            for (let entry of Array.from(
                                keywords_list.children
                            )) {
                                let item = entry.firstElementChild;
                                if (item === null) {
                                    console.warn("ERROR: item was null.");
