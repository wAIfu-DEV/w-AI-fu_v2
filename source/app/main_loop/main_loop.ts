...
export async function mainLoop_impl(): Promise<void> {
    ...
    const prompt = wAIfu.state.memory.getMemories(wAIfu.state.config._.context.value, user_prompt)
                   + character.char_name + ':';
    ...
    if (filtered_content !== null)
        ...
        wAIfu.state.memory.addMemory((is_monologue === true)
            ? `${wAIfu.state.config._.character_name.value}:${llm_response.value}`
            : `${user_prompt}${wAIfu.state.config._.character_name.value}:${llm_response.value}`);

    IO.quietPrint(`Answering to:\n${message.sender}: ${message.content}\n${wAIfu.state.config._.character_name.value}: ${llm_response.value.trim()}`);
    wAIfu.dependencies!.ui?.send('MESSAGE_CHAR', {text: llm_response.value.trim() });
