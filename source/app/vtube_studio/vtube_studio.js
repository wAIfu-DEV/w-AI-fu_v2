...
class VtubeStudioAPI {
        ...
        this.#websocket.on("message", (data) => {
            ...
            switch (message["messageType"]) {
                case MESSAGE_TYPE.AUTH_SUCCESS:
                    {
                        ...
                        Waifu_1.wAIfu.state.config._.vts_session_token.value =
                            this.#authToken;
                ...
                case MESSAGE_TYPE.ERROR:
                    {
                        io_1.IO.warn("ERROR: VtubeStudio API sent: " +
    ...
    async reset() {
        ...
        if (emotion === null) {
            io_1.IO.debug("ERROR: Tried to play undefined emotion: " +
                this.current_emotion);
            return;
        }
    ...
    animateSinging() {
        if (this.#websocket.readyState !== ws_1.default.OPEN)
            return;
        let steps = Waifu_1.wAIfu.state.config.vts.singing_hotkey_sequence.value;
        for (let step of steps) {
            this.#websocket.send(JSON.stringify({
                apiName: "VTubeStudioPublicAPI",
                apiVersion: "1.0",
    ...
    tryPlayKeywordSequence(text) {
        const val_array = Waifu_1.wAIfu.state.config.vts.keyword_based_animations.value;
