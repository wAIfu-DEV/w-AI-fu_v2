...
import { TextToSpeech, TTS_GEN_ERROR, TTS_PLAY_ERROR, TtsGenerationSettings, TtsPlaySettings } from "./tts_interface";
...
export class TextToSpeechNovelAI implements TextToSpeech {
    ...
    constructor() {
        ...
        this.#websocket_server = new WebSocketServer({ host: '127.0.0.1', port: 8766 });
    }
    ...
    async generateSpeech(text: string, options: TtsGenerationSettings): Promise<Result<string, TTS_GEN_ERROR>> {
            ...
            const el = (ev: WebSocket.MessageEvent) => {
                ...
                switch(split_data[1]) {
                    case "ERROR": {
                        ...
                        let err_msg = split_data.slice(3, undefined)
                                                .join(' ');
                        resolve(new Result(false, err_msg, err_type as TTS_GEN_ERROR));
                    } break;
                    default: {
                        let return_id = split_data.slice(1, undefined).join('');
                        resolve(new Result(true, return_id, TTS_GEN_ERROR.NONE));
                    };
                }
    ...
    async playSpeech(id: string, options: TtsPlaySettings): Promise<Result<void, TTS_PLAY_ERROR>> {
        await_connect: while(true) {
            if (this.#websocket.readyState === WebSocket.OPEN)
                break await_connect;
            await new Promise<void>((resolve) => setTimeout(() => resolve(), 100));
        ...
        return new Promise((resolve) => {
            this.#websocket.send("PLAY " + JSON.stringify({
                "id": id,
                "options": {
