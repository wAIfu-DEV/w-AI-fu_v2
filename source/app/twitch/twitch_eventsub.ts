...
export class TwitchEventSubs {
    ...
    async getTwitchAppAccessToken(): Promise<string> {
        ...
        if (resp["access_token"] === undefined) {
            IO.warn('Failed to get Twitch App Access Token. This may be due to incorrect Twitch App ClientId or Secret.');
            return '';
        }
    ...
    subscribeToEventSub(event_name: string, version: string, condition: any = {}, user_token: string, session_id: string): void {
        ...
        let create_ws = (url: string|null = null) => {
            ...
            ws.on('open', () => {
                ...
                ws.on('message', (data: WebSocket.RawData, _is_bin) => {
                    ...
                    switch (msg_type) {
                        case MSG_WELCOME: {
                            IO.debug('Successfully connected to Twitch EventSub WebSocket.');
                        ...
                        case MSG_RECONNECT: {
                            IO.warn('Received reconnection message from Twich EventSub WebSocket.');
                ...
                ws.on('close', (code: number, reason: Buffer) => {
                    IO.print(`Closed Twitch Events WebSocket with message: ${code} ${reason.toString("utf8")}`);
                    this.websocket = null;
                });
            });
        };
