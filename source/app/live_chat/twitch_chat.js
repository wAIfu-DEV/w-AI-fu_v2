...
class LiveChatTwitch {
    ...
    initialize() {
        return new Promise((resolve) => {
            ...
            this.#websocket.on("open", () => {
                ...
                this.#websocket.send(
                    `PASS oauth:${Waifu_1.wAIfu.state.auth.twitch.oauth_token}`
                );
                this.#websocket.send(
                    `NICK ${Waifu_1.wAIfu.state.auth.twitch.channel_name.toLowerCase()}`
                );
                this.#websocket.send(
                    `JOIN #${Waifu_1.wAIfu.state.auth.twitch.channel_name.toLowerCase()}`
                );
            });
            ...
            this.#websocket.on("message", (data) => {
                ...
                if (parsed_data.message.content.startsWith("!")) {
                    ...
                    (0, command_handler_1.handleCommand)(
                        parsed_data.message.content,
                        false
                    );
                    return;
    ...
    free() {
        return new Promise((resolve) => {
            if (
                this.#websocket.readyState !== ws_1.default.CLOSED &&
    ...
    send(message) {
        this.#websocket.send(
            `PRIVMSG #${Waifu_1.wAIfu.state.auth.twitch.channel_name
                .trim()
                .toLowerCase()} :${message}`
        );
    }
    ...
    #policyLatestBuffered() {
        let BUFF_SIZE =
            Waifu_1.wAIfu.state.config.live_chat.message_buffer_size.value;
        while (true) {
    ...
    #getWeight(msg) {
        const PRIO_RETURNING =
            Waifu_1.wAIfu.state.config.live_chat
                .prioritize_returning_viewers_message.value;
        ...
        if (
            PRIO_RETURNING &&
            chatter_status_1.ChatterInfos.isJustReturningChatter(msg.sender)
        ) {
            weight += 0.4;
        }
