...
export async function playSongPreprocessed(song_name: string): Promise<void> {
    ...
    if (fs.existsSync(SONGS_PATH) === false) {
        IO.warn('Warning: Could not find the folder userdata/songs. w-AI-fu will create a new one.');
        try {
            fs.mkdirSync(SONGS_PATH, { recursive: true });
        } catch {
            IO.error('Could not create the folder userdata/songs. Something went very wrong.');
        }
    }

    if (fs.existsSync(vocals) === false) {
        IO.warn('Error: Could not find file: ' + vocals);
        IO.print('Reminder: Song must be split in two files,\n<SONG NAME>_vocals.wav and <SONG NAME>_instrumentals.wav\nFiles must be placed in the folder userdata/songs.');
        return;
    }
    ...
    let player1 = cproc.spawn('python', [__dirname + '/sing.py', vocals, String(getDeviceIndex(wAIfu.state?.config.devices.tts_output_device.value!)), String(0)], { cwd: __dirname, detached: false, shell: false });
    let player2 = cproc.spawn('python', [__dirname + '/sing.py', instrumentals, String(getDeviceIndex(wAIfu.state?.config.devices.alt_output_device.value!)), String(1)], { cwd: __dirname, detached: false, shell: false });
