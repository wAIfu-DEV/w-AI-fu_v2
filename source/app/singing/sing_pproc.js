...
async function playSongPreprocessed(song_name) {
    ...
    await Waifu_1.wAIfu.dependencies?.vts.reset();
    Waifu_1.wAIfu.dependencies?.vts.animateSinging();
    ...
    let player1 = cproc.spawn(Waifu_1.ENV.PYTHON_PATH, [
        __dirname + "/sing.py",
        vocals,
        String((0, devices_1.getDeviceIndex)(Waifu_1.wAIfu.state?.config.devices.tts_output_device.value)),
        String(0),
    ], { cwd: __dirname, detached: false, shell: false });
    let player2 = cproc.spawn(Waifu_1.ENV.PYTHON_PATH, [
        __dirname + "/sing.py",
        instrumentals,
        String((0, devices_1.getDeviceIndex)(Waifu_1.wAIfu.state?.config.devices.alt_output_device.value)),
        String(1),
    ], { cwd: __dirname, detached: false, shell: false });
