...
export function pickRandomSong(last_song: string): string {
    let files: string[] = fs.readdirSync(process.cwd() + "/userdata/songs/", {
        recursive: false,
        withFileTypes: false,
        encoding: "utf8",
    });
    ...
    if (files.length === 0) {
        IO.warn(
            "ERROR: Could not pick random song since there are no songs in the userdata/songs folder"
        );
        return "";
    }
    ...
    while (++tries < 10) {
        ...
        if (split_name[0] === last_song) continue;
