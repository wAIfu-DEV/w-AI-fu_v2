...
if errorlevel 1 goto RetryPythonDowngrade
goto AfterPython

:RetryPythonDowngrade
echo Could not find python env variable, trying with direct path, but with downgraded version
...
%LOCALAPPDATA%\Programs\Python\Python311\Scripts\pip.exe --version 2>NUL
if errorlevel 1 goto RetryPipDowngrade
...
::ERROR HANDLING
:ErrPython
echo Could not find an installed Python environment. Please install Python (prefer 3.11.4) from the official website: https://www.python.org/downloads/
pause
goto:eof

:ErrPip
echo Could not find pip. Please install Python (prefer 3.11.4) with the 'pip' option checked. Or check: https://pip.pypa.io/en/stable/installation/
pause
goto:eof
